...
use tracing::info;
use zbus::{proxy, proxy::MethodFlags, zvariant, Connection};
...
/// Calls a `Manager` method with interactive polkit authorization enabled.
///
/// The proxy methods generated by `#[proxy]` send their D-Bus calls *without*
/// the `ALLOW_INTERACTIVE_AUTH` message flag. For polkit-protected operations
/// (start/stop/restart/enable/disable units, daemon reload) that means polkit
/// rejects the call outright with `InteractiveAuthorizationRequired` instead of
/// asking a registered polkit agent to authenticate the user. Setting the flag
/// lets a graphical agent (or `pkttyagent`) prompt for credentials, mirroring
/// what the `systemctl` CLI does by default.
///
/// `body` is the method's argument tuple (e.g. `&(name, mode)`), `R` its reply
/// type. Read-only methods don't need this, so they keep using the typed proxy.
async fn call_interactive<B, R>(proxy: &ManagerProxy<'_>, method: &str, body: &B) -> Result<R>
where
  ...
  R: for<'d> zvariant::DynamicDeserialize<'d>,
{
  proxy
    ...
    .await?
    .ok_or_else(|| anyhow::anyhow!("no reply received for {method}"))
}
...
pub async fn start_service(service: UnitId, cancel_token: CancellationToken) -> Result<()> {
  async fn start_service(service: UnitId) -> Result<()> {
    ...
    let _: zvariant::OwnedObjectPath =
      call_interactive(&manager_proxy, "StartUnit", &(service.name.clone(), "replace")).await?;
    Ok(())
  }
...
pub async fn stop_service(service: UnitId, cancel_token: CancellationToken) -> Result<()> {
  async fn stop_service(service: UnitId) -> Result<()> {
    let connection = get_connection(service.scope).await?;
    let manager_proxy = ManagerProxy::new(&connection).await?;
    let _: zvariant::OwnedObjectPath = call_interactive(&manager_proxy, "StopUnit", &(service.name, "replace")).await?;
    Ok(())
  }
...
pub async fn reload(scope: UnitScope, cancel_token: CancellationToken) -> Result<()> {
  async fn reload_(scope: UnitScope) -> Result<()> {
    ...
    call_interactive::<_, ()>(&manager_proxy, "Reload", &()).await.context(error_message)?;
    Ok(())
  }
...
pub async fn restart_service(service: UnitId, cancel_token: CancellationToken) -> Result<()> {
  async fn restart(service: UnitId) -> Result<()> {
    ...
    let manager_proxy = ManagerProxy::new(&connection).await?;
    let _: zvariant::OwnedObjectPath =
      call_interactive(&manager_proxy, "RestartUnit", &(service.name, "replace")).await?;
    Ok(())
  }
...
pub async fn enable_service(service: UnitId, cancel_token: CancellationToken) -> Result<()> {
  async fn enable(service: UnitId) -> Result<()> {
    ...
    let (_, changes): (bool, Vec<(String, String, String)>) =
      call_interactive(&manager_proxy, "EnableUnitFiles", &(files, false, false)).await?;
    ...
    call_interactive::<_, ()>(&manager_proxy, "Reload", &()).await?;
    Ok(())
  }
...
pub async fn disable_service(service: UnitId, cancel_token: CancellationToken) -> Result<()> {
  async fn disable(service: UnitId) -> Result<()> {
    ...
    let changes: Vec<(String, String, String)> =
      call_interactive(&manager_proxy, "DisableUnitFiles", &(files, false)).await?;
